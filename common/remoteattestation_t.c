...
static void ecall_verify_report_and_set_pubkey(
    uint8_t* input_buffer,
    size_t input_buffer_size,
    uint8_t* output_buffer,
    size_t output_buffer_size,
    size_t* output_bytes_written)
{
    ...
    /* Set in and in-out pointers. */
    if (pargs_in->pem_key)
        OE_SET_IN_POINTER(pem_key, ((size_t)pargs_in->key_size * sizeof(uint8_t)), uint8_t*);
    if (pargs_in->remote_report)
        OE_SET_IN_POINTER(remote_report, ((size_t)pargs_in->remote_report_size * sizeof(uint8_t)), uint8_t*);
